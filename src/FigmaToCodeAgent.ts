...
export class FigmaToCodeAgent {
  ...
  constructor(private config: AgentConfig) {
    ...
    this.pipeline.register(new FigmaStructureOptimizer()); // 1. Light optimization, preserve structure
  ...
  private collectImageNodes(
    node: import('./transformation/types').ASTNode,
    result: import('./transformation/types').ASTNode[]
  ): void {
    if (node.type === 'Image') {
      result.push(node);
    }
