...
export class ReactGenerator implements CodeGenerator {
  ...
  private buildImageImportMap(node: ASTNode): void {
      if (node.type === 'Image' && node.metadata.imageRef && node.metadata.imageRef.startsWith('./')) {
...
`
  ...
  private generateTailwindClasses(node: ASTNode): string {
    ...
    if (node.styles.borderRadius) {
      const radius = Array.isArray(node.styles.borderRadius) ? node.styles.borderRadius[0] : node.styles.borderRadius
  ...
  private collectStyles(node: ASTNode, styles: string[], parentNode?: ASTNode): void {
        ...
        const parentIsSingleTextContainer = parentNode && parentNode.children.length === 1 && node.type === 'Text'

        const hasNonFlexChildren = node.children.length > 0 && node.layout.display !== 'flex' && !isSingleTextParent

        // Flex layout (from Figma auto-layout)
        if (node.layout.display === 'flex') {
          cssRules.push(`  display: flex;`)
          if (node.layout.flexDirection) cssRules.push(`  flex-direction: ${node.layout.flexDirection};`)
          if (node.layout.justifyContent) cssRules.push(`  justify-content: ${node.layout.justifyContent};`)
        ...
        if (node.type === 'Text' && node.styles.typography) {
          ...
          if (t.letterSpacing && t.letterSpacing !== 0) cssRules.push(`  letter-spacing: ${t.letterSpacing}px;`)
          if (t.textAlign && !parentIsSingleTextContainer) cssRules.push(`  text-align: ${t.textAlign};`)
